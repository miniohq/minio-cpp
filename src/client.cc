...
namespace minio::s3 {
  ...
  StatObjectResponse Client::CalculatePartCount(size_t& part_count, std::list<ComposeSource> sources) {
  ...
  PutObjectResponse Client::PutObject(PutObjectArgs args) {
    ...
    char *buf;
    int res = posix_memalign((void **)&buf, getpagesize(), (args.part_count > 0) ? args.part_size : args.part_size + 1);
    if (res) {
